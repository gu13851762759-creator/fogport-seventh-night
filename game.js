...
let modalAction=()=>{};
const pop=(text,type='warning',onOk=()=>{})=>{$('#popup-sfx').currentTime=0;$('#popup-sfx').play().catch(()=>{});$('.pixel-modal').className='pixel-modal '+type;$('#modal-copy').innerHTML='<span class="pixel-icon">'+(type==='error'?'✖':'⚠')+'</span>'+text.replace(/\n/g,'<br>');modalAction=onOk;$('#modal-ok').textContent='收到';$('#modal-cancel').classList.add('hidden');$('#system-modal').classList.remove('hidden')};
...
const save=()=>localStorage.setItem(KEY,JSON.stringify(s)),go=x=>{s.scene=x;save();render()},btn=a=>{C.innerHTML='';a.forEach(([x,f])=>{let b=document.createElement('button');b.className='choice';b.textContent=x;b.onclick=f;C.append(b)})},add=x=>{if(!s.clues.includes(x))s.clues.push(x);save()};
const systemCopy={
  '毒舌':{
...
const sys=(kind,extra='')=>{const persona=systemCopy[s.persona]||systemCopy['梗王'];return '<section class="system system-'+(s.persona||'梗王')+'"><div class="system__head">系统 / '+(s.persona||'梗王')+'</div><p>'+resolveSystemCopy(persona,kind)+'</p>'+(extra?'<p class="system__data">'+extra+'</p>':'')+'</section>'};
const systemPop=(kind,title,extra='',type='warning')=>{const persona=systemCopy[s.persona]||systemCopy['梗王'];pop('【'+title+'】\n'+resolveSystemCopy(persona,kind)+(extra?'\n\n'+extra:''),type)};
...
if(s.scene==='bound'){T.textContent='人生系统 / 绑定完成';B.innerHTML='<div class="fireworks">✦　✧　✦　✧　✦</div><p class="binding">恭喜，绑定成功。</p>'+sys('binding','试炼世界：<b>《雾港第七夜》</b><br>初始污染度：<b>'+s.pollution+'%</b><br>同行系统：<b>'+s.persona+'</b>')+'<p>现在可以投入世界。</p>';btn([['开始投放',()=>{s.powered=false;s.scene='transfer';save();render();setTimeout(()=>{s.powered=true;go('identity')},900)}]]);setTimeout(()=>systemPop('binding','绑定成功','试炼世界：<b>《雾港第七夜》</b><br>请确认即将载入的身份。'),120);return}
...
$('#power-button').onclick=()=>{if(s.powered)return;$('#click-sfx').play().catch(()=>{});$('#ambience').volume=.32;$('#ambience').play().catch(()=>{});s.powered=true;go('boot')};
