...
$('#power-button').onclick=()=>{if(s.powered)return;playOne('#click-sfx');playOne('#startup-sfx');s.powered=true;music.volume=.30;rainAudio.volume=.12;setSoundscape('intro');go('boot')};
$('#modal-cancel').onclick=()=>{$('#system-modal').classList.add('hidden');modalAction=()=>{}};
$('#capture-store').onclick=()=>captureStoreAction();
$('#rewind-button').onclick=()=>{if(!s.powered)return;$('#rewind-button').classList.remove('rewind-guide');rewindTransition(()=>{pop('【回溯确认】\n确定要回溯到最初绑定吗？\n\n当前本机进度将被清空。','warning',()=>{localStorage.removeItem(KEY);location.reload()});$('#modal-ok').textContent='确认回溯';$('#modal-cancel').classList.remove('hidden');$('#modal-cancel').textContent='取消'})};
